...
class Vault extends Spawnable {
	...
	public function onUpdate() : bool{
		...
		if($tick % 5 === 0 && empty($this->itemsToEject)){
			$blockBB = $block->getCollisionBoxes()[0];

			$deactivationBox = $blockBB->grow($this->deactivationRange, $this->deactivationRange, $this->deactivationRange);
			...
			$activationBox = $blockBB->grow($this->activationRange, $this->activationRange, $this->activationRange);
	...
	public function tryOpen(Player $player, Item $itemUsed) : bool{
		...
		if(!$itemUsed instanceof TrialKey){
			return false;
		}
